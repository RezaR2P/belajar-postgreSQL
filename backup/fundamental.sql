...
values('P0009', 'Bakso a', 'Baso Di tambah mie ayam', 25000, 4)

-- kita juga bisa mengisi data sekaligus lebih dari satu
-- mengisi data tabel menggunakan insert
insert into products (id, name, price, quantity)
values ('P0004', 'Batagor', 7000, 100),
('P0003', 'Mie Ayam', 10000, 100)

-- mengambil data menggunakan perintah select
...
--    Contoh:
    SELECT * FROM products p  WHERE price >= 15000 AND name = 'Baso';
...
--    Contoh:
    SELECT * FROM products p  WHERE price >= 15000 OR name = 'Baso';
...
SELECT * FROM products p  WHERE (price >=15000 AND "name" = 'Baso') OR quantity > 10;
--Ini berarti "ambil semua data dari products di mana memenuhi (kondisi1 dan kondisi2) atau memenuhi kondisi3."


insert into products (id,name,price,quantity,category) values ('P0006','Es Teh Panas',20000,1000,'Minuman'),
('P0007','Es Kepala Muda',20000,1000,'Minuman'),
...
select * from products where name ilike '%m';
...
select * from products p where description is null;
...
select * from products p where price between 15000 and 20000;
select * from products p where price not between 15000 and 20000;
...
select * from products p where category in ('Makanan');
select * from products p where category not in ('Makanan');
...
select * from products p order by price asc, id desc;
...
select * from products p where price > 5000 order by price asc;
select * from products p where price > 5000 order by price asc, id desc  limit 2;
select * from products p where price > 5000 order by price asc, id desc  limit 2 offset 2;
...
select id, price / 1000 as price_in_k from products p ;
...
select * from t_products tp  ;
select * from t_order to2  ;


select 
	tor.id,tor.barang_id as kode_barang,tor.quantity,tp.name,tp.description
from 
	t_order tor inner join t_products tp 
	on tor.barang_id = tp.id;

select 
	tor.id,tor.barang_id as kode_barang,tor.quantity,tp.name,tp.description
from 
	t_order tor RIGHT join t_products tp 
	on tor.barang_id = tp.id;
